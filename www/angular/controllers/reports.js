...
app.controller('reports_date_wise', ['$scope','fns','seven','$state','$filter',
    function ( $scope , fns , seven , $state, $filter ) {
        ...
        $scope.toggleSelectiones = function (company_id) {
        ...
        $scope.filter_now = function() {
                ...
                if($scope.filter.company.length == 0 ){
                    ...
                    qry = 'SELECT sale_date AS date, IFNULL(SUM(amount_total),0) AS total_sales FROM sales WHERE company_id IN ('+$scope.filter.company+') AND sale_date BETWEEN "'+$scope.filter.start_dater+'" AND "'+$scope.filter.end_dater+'" GROUP BY date';
                }
                console.log(qry);
...
//
// Date wise Controller
app.controller('reports_comparison', ['$scope','fns','seven','$state','$filter',
    function ( $scope , fns , seven , $state, $filter ) {
        $scope.filter = {}
        $scope.filter.start_date      = '';
        $scope.filter.end_date      = '';
        ...
        $scope.filter_now = function() {
                ...
                if($scope.filter.start_date == '' || $scope.filter.end_date == '' || $scope.filter.company == '' ){
                    seven.alert('Please enter all values');
                    return;
                }

                $scope.filter.start_dater   = $filter('date')(new Date($scope.filter.start_date), "yyyy-MM-dd");
                $scope.filter.end_dater     = $filter('date')(new Date($scope.filter.end_date), "yyyy-MM-dd");
                    qry = 'SELECT sale_date AS date, IFNULL(SUM(amount_total),0) AS total_sales FROM sales WHERE company_id = '+$scope.filter.company+' AND sale_date IN ("'+$scope.filter.start_dater+'","'+$scope.filter.end_dater+'") GROUP BY date';
                console.log(qry);
